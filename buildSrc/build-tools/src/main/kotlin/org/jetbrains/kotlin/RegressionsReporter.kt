...
open class RegressionsReporter : DefaultTask() {
    ...
    private fun getCommits(buildLocator: String, user: String, password: String): /*CommitsList*/ Any{
        val changes = try {
            sendGetRequest(changesListUrl(buildLocator), user, password)
        } catch (t: Throwable) {
            error("Try to get commits! TeamCity is unreachable!")
        }
        return TODO("fixme")//CommitsList(JsonTreeParser.parse(changes))
    }
